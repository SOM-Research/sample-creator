...
def analyze_df(df): 
    ...
    return df
def check_mixed_columns(df):
    # Ignore the first column because is the name
    df = df.iloc[:, 1:]
    
    # Select categorical columns (including booleans)
    categorical_columns = df.select_dtypes(include=['object', 'bool']).columns.tolist()
    # Select numerical columns
    numerical_columns = df.select_dtypes(include=[np.number]).columns.tolist()
    
    # Check if all columns are numerical
    if numerical_columns and not categorical_columns:
        return 1
    # Check if all columns are categorical
    elif categorical_columns and not numerical_columns:
        return 2
    # Check if there is a mix of categorical and numerical columns
    elif categorical_columns and numerical_columns:
        return 3
    else:
        return None
...
def preprocessing(df):
    ...
    # Determine the type of variables we have
    type_variables = check_mixed_columns(df)
    ...
    if type_variables == 1:
    ...
    elif type_variables == 2 or type_variables == 3:
        if type_variables == 2:
            # All variables are categorical, use 'variables'
            variables = create_variables_dict_from_df(df_clean)
            ...
            # Mixed variables, use 'categorical_variables'
            # Separate numerical and categorical variables
            numerical_variables, categorical_variables = separate_numerical_categorical(df_clean)
        ...
        print("Could not determine the type of columns.")

    # Finally, we compute and print the statistical variables
    statistics = print_and_collect_statistics(variables)

    return numerical_variables, categorical_variables, counters, all_keys
...
print(df) 
numericcal_variables, categorical_variables, coutners, all_keys = preprocessing(df)
