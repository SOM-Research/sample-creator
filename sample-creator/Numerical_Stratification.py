...
def create_lists_from_df(df, columns=None): # SAME AS "create_variables_dict_from_df"
    ...
    return column_lists
def dictionary_to_lists(dictionary): # NOT USED
    """
    ...
    # Return the lists
    return keys, values
def count_elements_in_lists(lists_of_values): # NOT USED. NO SE SI ES NECESARIO
    """
...
def print_and_collect_statistics(variables): # REVIEW PRINTS.
    """
...
def create_stratum_kmeans(variables, num_clusters_list):
    ...
    return strata
def print_stratum_counts(strata): # CANDIDATE FOR UTILS/STRATIFICATION
    """
...
#COMBINATION
def get_stratum_ranges(strata): #ONLY NUMERICALS
    """
    ...
    return ranges
def combination(ranges): #ONLY NUMERICALS
    """
    ...
    return combinations
def df_to_list_observations(df): # PRIVADO, FOR BOTH
    """
...
def classify_observations(observations, strata_ranges): #ONLY NUMERICALS
...
def drop_empty_strata(classified_observations, strata_combination): # ONLY NUM. PRIVATE.
    """
...
def print_combination_stratum_counts(classified_observations, strata_combinations): # CANDIDATE TO REMOVE. PRINT STRATUM. ONLY NUMERICAL, PERO PODRIA SER DE CATEGORICAL
    """
...
def nis_phi(classified_observations, N): # SAME FOR CAT, NUM, PHI
    ...
    return nis, phi
def calculate_variable_std_devs(classified_observations): # ONLY FOR NUMERICALS
    """
...
def nStratifiedSampling(epsilon, confidence, phi, s, setting, N, nis): # ONLY FOR NUMERICALS
    """
    ...
    return n, n_strata
def calculate_sample_sizes(mu, confidence, phi, all_s, setting, N, nis): # ONLY FOR NUMERICALS
    """
    JUNE: TO FILL
    """
    sample_sizes = []
...
def get_max_sample_distribution(sample_sizes, strata): # ONLY FOR NUMERICALS
    """
...
def filter_zero_strata(max_n_dist, phi, nis, s, max_n_idx, classified_observations): # ONLY FOR NUMERICALS
    """
...
# SAMPLING
def sampling(classified_observations, index, phi_list, nis_list, s_list, max_n_idx, max_n, max_n_dist): # CAN'T BE REMOVED, CALLED IN AUTOMATED SAMPLING. ONLY FOR NUMERICALS
    """
...
def automated_sampling(classified_observations, phi, nis, s, max_n_idx, max_n, max_n_dist): # ONLY FOR NUMERICALS
    """
...
# observations = df_to_list_observations(df_clean) # SE LLAMARÁ DESDE CLASSIFY_OBSERVATIONS
